...
import { UserCredentials } from "@/app/hooks/useUser";
...
const Accordion = () => {
    ...
    useEffect(() => {
        const user: UserCredentials = JSON.parse(String(getItem("user")));

        getNavLinks(user.token).then(response => {
            if(!response.ok){
    ...
    return (
        <>
            <Link 
                key="dashboard"
                className="w3-bar-item w3-button w3-block w3-round w3-hover-light-gray w3-left-align"
                href='/dashboard' >
            ...
            </Link>
            {
                Object.keys(seccionMenu).map((menu) => {
                    ...
                    return <div key={ menu }>            
                        <button onClick={() => myAccordion(parseStr) } className="w3-button w3-block w3-round w3-hover-light-gray w3-left-align">
                            <p>
